...
function Absen($data)
{
  ...
  $absen = htmlspecialchars($data["absen"]);
  $tgl = date("Y-m-d");
  $cek = mysqli_query($conn, "SELECT * FROM tb_absen WHERE nisn = '" . $nisn . "' AND tgl_absen = '" . $tgl . "'");
  if (mysqli_num_rows($cek) > 0) {
    echo "<script>alert('Siswa Hari Ini Sudah Pernah Absen')</script>";
    return false;
  } else {
    //ambil Id terbesar di kolom id pendaftaran,lalu ambil 5 karakter aja dari sebelah kanan 
...
function hapusSiswa($id)
{
  ...
  //disimpanya gambar
  $siswa = mysqli_query($conn, "SELECT gambar FROM tb_siswa WHERE nisn ='" . $id . "' ");
...
function hapusKelas($id)
{
  ...
  //menghapus file gambar di db sekaligus difoldernya
  //disimpanya gambar
  $kelas = mysqli_query($conn, "SELECT logo FROM tb_kelas WHERE id_kelas ='" . $id . "' ");
  //mengecek data di table
  if (mysqli_num_rows($kelas) > 0) {
    $p = mysqli_fetch_object($kelas);
    //mengecek data difolder
    if (file_exists("logo/" . $p->logo)) {
      //jika ada difolder jurusan maka lakukan proses hapus file
      unlink("logo/" . $p->logo);
    }
  }
...
function hapusOfficer($id)
{
  ...
  //menghapus file gambar di db sekaligus difoldernya
  //disimpanya gambar
  $officer = mysqli_query($conn, "SELECT foto FROM tb_admin WHERE admin_id ='" . $id . "' ");
  //mengecek data di table
  if (mysqli_num_rows($officer) > 0) {
    $p = mysqli_fetch_object($officer);
    //mengecek data difolder
    if (file_exists("foto/" . $p->foto)) {
      //jika ada difolder jurusan maka lakukan proses hapus file
      unlink("foto/" . $p->foto);
    }
  }
